...
async fn repeat_deck(deck_name: &str) {
    ...
    let random_nums = generate_random(deck_name).await;

    for i in random_nums {
        let need_struct = loop {
            for i_i in &vector {
                if i == i_i.num {
                    found = Some(i_i);
                    break;
...
async fn generate_random(deck_name: &str) -> Vec<usize>{
    let path = format!("./decks_of_card/{}", deck_name);
    ...
    if line_count <= 1 {
        panic!("File is empty! Check {path}")
    }
    let mut numbers: Vec<usize> = (1..=line_count).collect();
